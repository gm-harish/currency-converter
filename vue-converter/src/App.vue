...
export default class VwapStatsPane extends Vue {
  ...
  private eurValueChanged(value: any, currency: string) {
    ...
    this.eurValue = parsedValue
      ? parsedValue / this.conversionMap[currency]
      : null;
  }
  ...
  private async fetchCurrencyConversionData() {
    try {
      const { data } = await axios.get(
        "http://data.fixer.io/api/latest?access_key=3a82e4b662de26098e211dfb00130f86&symbols=USD,INR,GBP"
      );
